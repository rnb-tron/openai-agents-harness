...
async def lifespan(_: FastAPI):
    initialized = {"http": False, "redis": False, "kafka": False, "database": False}
    ...
    try:
        await app.state.harness.setup()
    except Exception as e:
        logger.error(f"harness setup failed: {e}", exc_info=True)
        raise

    # Setup protocol-layer middleware plugins.
    try:
    ...
    try:
        await app.state.harness.teardown()
    except Exception as e:
        logger.error(f"harness teardown failed: {e}", exc_info=True)
...
app.state.harness = build_harness(current_settings)

# Register protocol-layer plugins.
# Order matters: Observability wraps Auth/RateLimit, Auth runs before RateLimit.
# (MiddlewareRegistry handles FastAPI's LIFO stack internally.)
_observability_plugin = ObservabilityPlugin.from_settings(current_settings)
if _observability_plugin.is_enabled():
    middleware_registry.register(_observability_plugin)
