...
_PREFERENCE_MEMORY_QUERY = "user preferences, stable instructions, communication style"
_PREFERENCE_MEMORY_SEARCH_LIMIT = 10
...
class Mem0MemoryManager:
    ...
    async def get_context(
        ...
        # 长期记忆开启时，偏好记忆是用户级常驻约束；它独立于本轮普通语义召回，
        # 即使当前输入很短或调用方关闭普通长期记忆检索，也会优先注入已生效偏好。
        ...
        if self._long_term_enabled() and enable_retrieval and self._should_retrieve(user_input):
            # 普通长期记忆只召回与本轮输入相关的 top_k，避免把全部长期记忆塞进 prompt。
    ...
    async def _get_preferences(self, user_id: str) -> list[dict[str, Any]]:
        ...
        try:
            # 偏好检索使用独立 limit，不能被普通长期记忆的上下文条数限制；
            # 后续只保留带 preference metadata 的结果，并按偏好维度选最新生效项。
            raw = await self._search_mem0(
                query=_PREFERENCE_MEMORY_QUERY,
                user_id=user_id,
                limit=_PREFERENCE_MEMORY_SEARCH_LIMIT,
            )
            results = self._normalize_search_results(raw)
            preference_candidates = [memory for memory in results if self._preference_keys(memory)]
            preferences = self._select_effective_preferences(preference_candidates)
            self._preference_cache[user_id] = (now + ttl, preferences)
            return preferences
        except Exception as exc:
            service_logger.warning(f"Mem0 preference search failed open: user={user_id}, error={exc}")
            self._preference_cache[user_id] = (now + ttl, [])
            return []
    ...
    async def list_memories(
        ...
        """列出用户长期记忆，用于管理 UI；不按当前输入做语义检索。"""
        if not self._long_term_enabled():
            return []
        try:
            raw = await self._get_all_mem0(user_id=user_id, limit=limit)
            memories = self._normalize_search_results(raw)[:limit]
            for memory in memories:
                # 这里的分类只服务 UI 展示分组。Mem0 local 可能不会给偏好记忆
                # 写入 preference metadata，但生成内容会是 “User prefers ...”；
                # prompt 注入和偏好冲突消解仍然只依赖 _preference_keys(metadata)。
                memory["memory_category"] = self._display_memory_category(memory)
            return memories
        except Exception as exc:
            service_logger.warning(f"Mem0 list failed open: user={user_id}, error={exc}")
            return []

    async def _get_all_mem0(self, *, user_id: str, limit: int) -> Any:
        try:
            return await self._call(
                "get_all",
            ...
            try:
                return await self._call("get_all", user_id=user_id, top_k=limit)
            except TypeError:
                return await self._call("get_all", user_id=user_id, limit=limit)
    ...
    @classmethod
    def _display_memory_category(cls, memory: dict[str, Any]) -> str:
        if cls._preference_keys(memory):
            return "preference"
        ...
        return "preference" if any(phrase in content for phrase in preference_phrases) else "long_term"
