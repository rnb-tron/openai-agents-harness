...
> 注意：本文保留 Manager 级组件示例用于调试与理解能力边界。应用接入 HITL 时，推荐使用 Runtime 已接入的 OpenAI Agents SDK 原生 `interruptions` / `POST /chat/resume` 流程；应用接入 Handoff 时，推荐使用配置驱动的 SDK 原生 `Agent.handoffs` 流程。
...
查看完整的端到端测试文件:
- `tests/e2e/test_advanced_agents_e2e.py` - 客服工单处理完整工作流

运行测试:
```bash
venv/bin/python -m pytest tests/e2e/test_advanced_agents_e2e.py -v
```

运行示例:
```bash
python examples/advanced_agents_components.py
```

HTTP 应用中的 SDK 原生审批恢复流程请使用：

```bash
venv/bin/python examples/hitl_resume.py --approve --message "请查询北京天气。"
```

该脚本要求服务已配置 `HITL_ENABLED=true` 和待审批工具，并展示 `/chat` 返回的 `interruptions` / `run_state` 如何交给 `/chat/resume`。
